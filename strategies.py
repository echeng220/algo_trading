...
class Double7Strategy(bt.Strategy):
    params = (
        ('name', 'Double 7s'),
        ('starting_balance', 10000)
    )
    ...
    def __init__(self):
        ...
        # Add SMA, 7-day low, 7-day high indicators
        self.sma = bt.indicators.SimpleMovingAverage(
            self.datas[0], period=200)
        self.low = bt.indicators.Lowest(self.datas[0], period=7)
        self.high = bt.indicators.Highest(self.datas[0], period=7)
    ...
    def buy_signal(self):
        below_low = self.dataclose[0] < self.low[-1]
        above_sma = self.dataclose[0] > self.sma[0]

        if above_sma and below_low:
            return True
        else:
            return False

    def sell_signal(self):
        above_high = self.dataclose[0] > self.high[-1]
        # above_sma = self.dataclose[0] > self.sma[0]
    ...
    def next(self):
        # Simply log the closing price of the series from the reference
        self.log(
            'Close: %.2f, SMA: %.2f, 7-day low: %.2f, 7-day high: %.2f' %
            (self.dataclose[0], self.sma[0], self.low[-1], self.high[-1])
        )
